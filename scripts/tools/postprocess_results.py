...
def parse_map_yaml(path: Path) -> Dict[str, Any]:
    """
    Minimal parser for simple ROS map YAML files without adding a YAML dependency.
    Expected keys include: image, resolution, origin, negate, occupied_thresh, free_thresh.
    """
    data: Dict[str, Any] = {}
...
def load_pgm(path: Path) -> np.ndarray:
    """Read P2 or P5 PGM into a 2D uint8 array."""
...
def world_to_map_pixel(
    ...
    """
    Approximate transform from world coordinates to occupancy-grid pixel coordinates.
    """
...
def find_map_pairs(folder: Path) -> List[Tuple[Path, Path]]:
    """Find matching .yaml + .pgm pairs inside a folder."""
...
def choose_map_pairs_for_run(run_dir: Path, slam_maps_root: Optional[Path]) -> List[Tuple[Path, Path]]:
    """
    Search for map outputs in:
      1) run_dir itself
      2) run_dir/maps
      3) slam_maps_root/run_name, if provided
    """
...
def overlay_trajectory_on_map(
    ax: Any,
    map_meta: Dict[str, Any],
    image: np.ndarray,
    trajectory_df: Optional[pd.DataFrame],
) -> None:
...
def make_snapshot_gallery(run_dir: Path, map_pairs: List[Tuple[Path, Path]], out_dir: Path) -> None:
    ...
    # Save per-frame images for optional animation later
